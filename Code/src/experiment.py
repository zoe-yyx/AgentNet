...
class Experiment:
    def __init__(self, 
                 ...
                 few_shot_example = APPSPromptSet.get_few_shot()
                 ):
        ...
        # self.test_dataset_unseen = test_dataset_unseen
        ...
        self.few_shot = few_shot_example
    ...
    def solve_a_single_task(self, task): 
        # Initial agent selection for each task
        # Nature of APPS: random selection, then classification 
        task_chain = TaskChain(task)
        forward_times = self.forward_path_max_length
        current_execution_times = 0
        # Randomly select an agent to start task execution
        current_agent_id = self.agent_graph.select_an_agent(task.task_type)
        # current_agent_id = self.agent_graph.select_an_agent(task_chain)
        current_agent = self.agent_graph.agents[current_agent_id]
        logger.info("="*8 + f"Task {task_chain.task_chain_id} Starts" + "="*8)
        logger.info(f"Agent {current_agent.agent_id} Decides Task Type [{task_chain.task_type}] for Task {task_chain.task_chain_id}")
        ...
        while True:
            ...
            if decision in ["forward"]:
                ...
                output = current_agent.execute_task(task_chain, self.constraints, self.thought_constraints, self.few_shot, self.user_react, decision="execute")
    ...
    def evaluate_task_result(self, task_chain, correct_answer, mode="Train"):
        ...
        # logger.info(f"raw_result from MAS is {raw_result}")
        final_result = extract_answer(raw_result)
        logger.info(f"Extracted Answer from MAS is {final_result}")
        logger.info(f"correct_answer from MAS is {correct_answer}")
        success = evaluate_answer(correct_answer, final_result)

        result = {
            "task_chain_id": task_chain.task_chain_id, 
            "raw_result": raw_result,
            "final_result": final_result,
                            # "correct_answer": correct_answer,  # Too long and meaningless
            # "single_success": success,
            "pass_rate": np.mean(np.asarray(success) > 0),
            "success": bool(np.all(np.asarray(success) > 0))
        } 

        logger.info(f"Extracted Answer from MAS is {success}")

        
        if mode == "Train":
            self.train_accuracy.update(success)
            logger.info(f"Test Case Average of All Agents on Train Dataset Is {self.train_accuracy.get_accuracy()[0]}, Strict Accuracy of All Agents on Train Dataset Is {self.train_accuracy.get_accuracy()[1]}")

        if mode == "Test":
            self.test_accuracy.update(success)
            logger.info(f"Test Case Average of All Agents on Train Dataset Is {self.test_accuracy.get_accuracy()[0]}, Strict Accuracy of All Agents on Train Dataset Is {self.test_accuracy.get_accuracy()[1]}")

        if mode == "Test_fit":
            self.test_accuracy2.update(success)
            logger.info(f"Test Case Average of All Agents on Train Dataset Is {self.test_accuracy2.get_accuracy()[0]}, Strict Accuracy of All Agents on Train Dataset Is {self.test_accuracy2.get_accuracy()[1]}")
    ...
    def update_agent_graph(self, task_chain, result):
        ...
        for single_task_history in task_history:
            ...
            if mode in ["Execution", "Split_Execution"]:
                # Update executing Agent's abilities: strengthen on success, no reduction on failure
        ...
        self.task_result_record.append(copy.deepcopy(result))

        self.edge_weight_record.append({
            "task_chain_id": copy.deepcopy(task_chain.task_chain_id), 
            "edge_weight": copy.deepcopy(self.agent_graph.edge_weight)
        })
        ...
        # logger.info(f"The Ability of All Agents Is {all_agent_info}")
        # for record in self.agent_info_record:
        #     logger.info(f"The Ability of All Agents Is {record}")  # just for debug
